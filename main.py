...
def initialize_upload(youtube, options):
    for filename in os.listdir(options.path):
        ...
        default_keywords = ""
        keywords = input("Enter the video tags (comma separated): ")
        description = input("Enter the video description: ")
        category = input("Enter the video category: ")
        privacyStatus = input("Enter the privacy status: ")
        ...
        upload_video(
            ...
            category=category,
            privacyStatus=privacyStatus or VALID_PRIVACY_STATUSES[1],
            file=os.path.join(options.path, filename)
        )


def upload_video(youtube, **kwargs):
    tags = None
    if kwargs["keywords"]:
        tags = kwargs["keywords"].split(",")
    ...
    body = dict(
        ...
        status=dict(
            privacyStatus=kwargs["privacyStatus"]
        )
    )
    ...
    insert_request = youtube.videos().insert(
        ...
        media_body=http.MediaFileUpload(kwargs["file"],
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.path):
        exit("Please specify a valid path using the --path= parameter.")
